...
const UID_DB = process.env.REACT_APP_GRAFANA_UID_DB || 'rYdddlPWk';
...
export default function DatabaseDashboards() {
  ...
  useEffect(() => {
    ...
    fetch(url)
      .then(async (r) => {
        ...
        const byUid = Array.isArray(items) ? items.find((i) => i.uid === UID_DB) : undefined;
        ...
        if (pick && pick.url) {
          ...
          setDashPath(urlPath); // e.g. /d/rYdddlPWk/database-metrics-...
        } else {
          setDashPath(`/d/${UID_DB}`);
        }
      })
      .catch((e) => {
        setProxyOk(false);
  ...
  return (
    <main className="p-4 md:p-8 w-full">
      ...
      <p className="mt-4 text-xs text-gray-600">Source: {(!proxyOk) ? 'direct' : 'proxy'} | UID: {UID_DB} | Path: {dashPath}</p>
      {DEBUG && (
        <div className="mt-3">
